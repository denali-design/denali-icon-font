...
document.getElementById("navDropdownToggle").addEventListener("click", function() {
  var element = document.getElementById("collapseDropdown");
...
function activeScroll() {
  ...
  for (var i = 0; i < navGrandChild.length; i++) {
    ...
    if (content.scrollTop >= document.getElementById(activeArea).offsetTop - 140 && content.scrollTop <= document.getElementById(
        activeArea).offsetHeight + document.getElementById(activeArea).offsetTop - 140) {
...
// Icon Outlines
var iconCategories, i, iconOutlineList = "",
  iconSolidList = "";
for (i in iconCategories) {
  ...
  for (j in iconCategories[i].icons) {
    iconOutlineList += '<a data-search-query="' + iconCategories[i].icons[j] + '" id="' + iconCategories[i].icons[j] + '" href="#' + iconCategories[i].icons[j] +
      '" class="icon-box flex flex-column">'
    iconOutlineList += '<span class="is-medium d-icon d-' + iconCategories[i].icons[j] + '"></span>'
...
for (i in iconCategories) {
  ...
  for (j in iconCategories[i].icons) {
    iconSolidList += '<a data-search-query="' + iconCategories[i].icons[j] + '-solid" id="' + iconCategories[i].icons[j] + '-solid" href="#' + iconCategories[i].icons[j] +
      '-solid" class="icon-box flex flex-column">'
    iconSolidList += '<span class="is-medium d-icon d-' + iconCategories[i].icons[j] + '-solid"></span>'
...
document.getElementById("content").onscroll = function() {shadowScroll()};
function shadowScroll() {
  if (document.getElementById("content").scrollTop > 0 || document.getElementById("content").scrollTop > 0) {
...
function populateDownloadModal(icon) {
  // alert(this.id);
  var iconName = icon;
  var downloadGroup = '<div class="input"><input type="text" value="d-'+ iconName +'"/></div><a href="downloads/png/'+ iconName +'.png" download class="button is-secondary">Download PNG</a><a href="downloads/svg/'+ iconName +'.svg" download class="button is-secondary">Download SVG</a>';
  var downloadTitle = '<i class="d-icon d-'+ iconName +'"></i> '+ iconName +''
...
function titleRemove() {
  ...
  for (i = 0; i < c.length; i++) {
    var txt = c[i].id;
    var count = document.getElementById(c[i].id).querySelectorAll('div a').length;
    var hideCount = document.getElementById(c[i].id).querySelectorAll('div a.hide').length;
...
function noResults() {
  ...
  var containerCountHidden = document.querySelectorAll('#iconContainer > div.hide').length;
...
var countTotal  = countOne*2 + countTwo*2 + countThree*2 + countFour*2 + countFive*2 + countSix*2 + countSeven*2 + countEight*2 + countNine*2 + countTen;
